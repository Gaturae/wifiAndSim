	...
	if(ack&&waittime)		//需要等待应答
	{
		while(--waittime)	//等待倒计时
		{
			...
			if(USART2_RX_STA&0X8000)//接收到期待的应答结果
			{
				...
				strx = atk_8266_check_cmd(ack);
				if(strx)
				{
					//printf("ack:%s\r\n",(u8*)ack);
					u1_printf("ack:%s\r\n",(u8*)strx);
					//u1_printf("ack:%s\r\n",(u8*)(strx+110));    //STAMAC
					//u1_printf("ack:%s\r\n",(u8*)(strx+118));      //18:fe:34:d2:8b:c7
					break;//得到有效数据 
				}
					USART2_RX_STA=0;
			} 
...
void Esp8226TcpLoad()
{
		...
		atk_8266_send_cmd("AT+CWMODE=3","",200);
		//while(atk_8266_send_cmd("AT+CWJAP=\"Joekoe_Office\",\"jkwlan1324\"","OK",1000));
	  //while(atk_8266_send_cmd("AT+CWJAP=\"SEA-PUBLIC\",\"ilovesea\"","OK",1000));
		while(atk_8266_send_cmd("AT+CWJAP=\"@PHICOMM_48\",\"guobaoqi\"","OK",1000));
		atk_8266_send_cmd("AT+CIFSR","",200);		

		//atk_8266_send_cmd("AT+CIPSTART=\"TCP\",\"192.168.10.157\",8080","",20);
	  while(atk_8266_send_cmd("AT+CIPSTART=\"TCP\",\"106.15.33.122\",8437","OK",20));
...
//查询wifi模块的MAC地址并且返回
u8 atk_8266_send_cmd_back(u8 *cmd,u8 *ack,u16 waittime,u8 MacName[6],u8 MacVaule[17])
{
	u8 res=0; 	
	u8 *data;
	int i,k;
	...
	if(ack&&waittime)		//需要等待应答
	{
		while(--waittime)	//等待倒计时
		{
			...
			if(USART2_RX_STA&0X8000)//接收到期待的应答结果
			{
				...
				data = atk_8266_check_cmd(ack);
				k = StringGet(data,"STAMAC");   //搜索指定字符串并返回其位置
				printf("position=%d",k);	
...
//判断是否正确收到MAC地址，并发送给服务器进行握手
void SendTcpHello(u8 MacName[6],u8 MacVaule[17])
{
	
	int k;
			
		if(CmdCmp(MacName,"STAMAC",6)==0)
		{
			  ...
			  u1_printf("tcp://");
				for(k=0;k<17;k++)
				{
					if(k%3==2)   //去除MAC地址中的冒号":"
					{
						continue;
					}
					else
					{
					u2_printf("%c",MacVaule[k]);    //MacVaule
					u1_printf("%c",MacVaule[k]);    //MacVaule
					}
...
//接收终端设备上报的数据
u8* device_msg_back(u8 mode)
{
	u8* data;
	if(USART_RX_STA&0X8000)		//接收到一次数据了
	{ 
		USART1_RX_BUF[USART_RX_STA&0X7FFF]=0;//添加结束符
		//u1_printf("%s",USART1_RX_BUF);
...
void dev_to_ser(u8 *Message,u8 MacVaule[17])
{
	int k;
	char i[3];  //~12XX@   上发剩余运行时间
	char temp[10];
	//u2_printf("message is %s\r\n",Message);
	...
	for(k=0;k<3;k++)
	{
		...
		//u2_printf("%c",i[k]);
			...
			//u2_printf("idlecnt=%d,workcnt=%d",idlecnt,workcnt);
	
	if((idlecnt==0)&&(StringCmp(temp, "~0100@") == 0))			//第一次上电
	{
		  ...
		  macPrint(MacVaule);
			u2_printf("#1060876800#0#000000#startup");
			u2_printf("$hashcode");
				
	}	
	else if((workcnt==0)&&(StringCmp(temp, "~1200@") == 0))	//每次启动命令发送后
	{
	...
	else if(StringCmp(temp, "~1100@") == 0)		//每次工作完成后待机状态
	{
			if((idlecnt!=0)&&(idlecnt==15))
			{
				idlecnt=1;
				u2_printf("tcp://");
				macPrint(MacVaule);
				u2_printf("/~");
				macPrint(MacVaule);
				u2_printf("#1060876800#0#000000#idle");
				u2_printf("$hashcode");
			}
			else if(idlecnt==0)
			{
	...
	else if((idlecnt!=0)&&(StringCmp(temp, "~1300@") == 0)) 	//每次工作完毕后发送一次
	{
				...
				u2_printf("/~");
				macPrint(MacVaule);
				u2_printf("#1060876800#0#000000#ended");
				u2_printf("$hashcode");
			
	}
			
	else if((StringCmp(i, "~12") == 0)&&(CmdCmp(temp, "~1200",5) != 0))		//工作过程中实时上报剩余时间
	{
				
			if((workcnt!=1)&&(workcnt==16))
			{
				workcnt=2;
				u2_printf("tcp://");
				macPrint(MacVaule);
				u2_printf("/~");
				macPrint(MacVaule);
				u2_printf("#1060876800#0#000000#work#");
				for(k=3;k<5;k++)
				{
...
u8* service_to_device(u8 mode)
{
	...
	{ 
		USART2_RX_BUF[USART2_RX_STA&0X7FFF]=0;//添加结束符
		data = USART2_RX_BUF;	
		k = StringGet(data,"startup");
		
		
		n = StringGet(data,"$");
		
		close = StringGet(data,"CLOSED");
		...
		if(k)
		{
			if(count==4)
			{
				...
				u1_printf("~60");
				u1_printf("%c",*(data+k+7));
				u1_printf("%c",*(data+k+8));
				u1_printf("@");
...
u8* service_to_device1(u8 mode)
{
	...
	{ 
		...
		count = n-k;
		//u1_printf("%s%c",data,count);
		u1_printf("1111111111111111");
		u1_printf("count=%c",count);
...
void esp8266_init(u8 MacVaule[17])
{
	u8 MacName[6];
	//启动WIFI模块序列
	while(atk_8266_send_cmd("AT","OK",20))//检查WIFI模块是否在线
	{
		...
		do
		{
			atk_8266_send_cmd_back("AT+CIFSR","",200,MacName,MacVaule);
			delay_ms(1000);			
		}while(CmdCmp(MacName,"STAMAC",6)==1);
		
	

	
		Esp8226TcpLoad();  //开启透传模式
	

		SendTcpHello(MacName,MacVaule);  //与服务器进行握手通讯
...
void esp8266_smart_init(u8 MacVaule[17])
{
	...
	while(atk_8266_send_cmd("AT","OK",20))//检查WIFI模块是否在线
	{
		...
		atk_8266_send_cmd("AT+CWMODE=1","",200);
		atk_8266_send_cmd("AT+RST","",20);
		delay_ms(5000);
		while(atk_8266_send_cmd("AT","OK",20));
		...
		while(atk_8266_send_cmd("AT+CWSTARTSMART","OK",200));		
		
		do
		{
				delay_s(2);			  
		}while(atk_8266_send_cmd("AT+CIPSTART=\"TCP\",\"106.15.33.122\",8437","OK",20));
		while(atk_8266_send_cmd("AT+CWSTOPSMART","OK",200));	
    while(atk_8266_send_cmd("AT+CIPMODE=1","OK",20));
		while(atk_8266_send_cmd("AT+CIPSEND","OK",20));
...
void sim900a_init(u8 GPRS_IMEI[17])
{
		...
		do
		{
			Message = atk_gprs_send_cmd_back("AT+COPS?","",200);
			delay_s(1);
		}while(StringGet(Message,"CHINA") == 0);
		atk_gprs_send_cmd_back("AT+CIPCLOSE","",200);
		atk_gprs_send_cmd_back("AT+CIPSHUT","",200);
		atk_gprs_send_cmd_back("AT+CIPMODE=1","",200);
		atk_gprs_send_cmd_back("AT+CSTT=\"CMIOT\"","",200);
		atk_gprs_send_cmd_back("AT+CIICR","",200);
		atk_gprs_send_cmd_back("AT+CIFSR","",200);
		Message = atk_gprs_send_cmd_back("AT+GSN","",200);		 //获取GPRS模块ID号	
		for(k=0;k<15;k++)
		{			
			GPRS_IMEI[k] = *(Message+10+k);
		}	
		do
		{
			Message = atk_gprs_send_cmd_back("AT+CIPSTART=\"TCP\",\"106.15.33.122\",\"8437\"","",200);
			delay_s(1);
		}while(StringGet(Message,"FAIL") != 0);
		u2_printf("tcp://");
		for(k=0;k<15;k++)
		{
			u2_printf("%c",GPRS_IMEI[k]);
		}	
		u2_printf("/~hello");
		delay_ms(100);
